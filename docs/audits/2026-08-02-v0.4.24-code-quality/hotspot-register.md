...
| P1 | `src/client/store/mcp-handler.js` / 默认 store mixin | 文件 1580 行；单闭包约 1480 行；18 处 `window.store` | 工具注册、取消、SSH/SFTP 适配和 store 原型扩展集中，隔离测试困难 | 按 transport、tool registry、SSH/SFTP adapter、store facade 四层拆分 | 登记；轮次五重点核对 AbortController 和后台 Promise 收口 |
| P1 | `src/client/components/sftp/sftp-entry.jsx` / `Sftp` | 类 2209 行，方法覆盖挂载、列表、编辑、删除、备份、恢复和渲染 | UI 状态、远端 IO 与安全事务在一个类中，任何修改回归面大 | 先抽 `sftp-list-controller` 与 `sftp-recovery-controller`，组件保留 view state；复用现有 transaction adapter | 登记；轮次五核对资源配对 |
