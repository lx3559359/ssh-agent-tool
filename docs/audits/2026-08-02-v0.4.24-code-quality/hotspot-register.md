...
- 已增加 Node/Vite engine 契约，避免声明支持实际无法构建的 Node 版本。
- 已增加 package-script 入口存在性与 README 脚本引用契约，清理 1 个死脚本和 2 处错误启动说明。
- 顶层依赖树完整，生产与全量 `npm audit` 均为 0 漏洞。
- 4 个混合 ESM/CommonJS 测试文件仍会产生 Node 类型提示，登记为 P3 测试结构债；应用根 package 不能直接切换为 ESM。
