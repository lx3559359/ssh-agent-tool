...
15. 远程侧默认采用无驻留架构；开启接管但没有运行任务时，不因 Agent 额外启动远程进程、不发送模型请求，也不周期轮询 SSH 服务器。
...
- 接管开关只是当前会话的执行授权。处于 `active-idle` 时不发送模型请求、不执行远程命令、不扫描文件或日志，也不为每个已开启会话启动周期轮询。
...
- 验证多个空闲接管会话不会因 Agent 产生额外远程进程、SSH 命令、模型请求或随会话数增长的周期工作。
...
21. 接管开启但没有运行任务时，不发送模型请求、不执行远程命令、不因 Agent 额外启动远程进程或周期轮询。
