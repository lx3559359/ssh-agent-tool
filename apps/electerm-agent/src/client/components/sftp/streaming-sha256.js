const initialState = Object.freeze([
  0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a,
  0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19
])

const roundConstants = Object.freeze([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
  0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3,
  0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
  0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7,
  0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13,
  0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3,
  0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5,
  0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
])

function rotateRight (value, bits) {
  return (value >>> bits) | (value << (32 - bits))
}

class StreamingSha256 {
  constructor () {
    this._state = Uint32Array.from(initialState)
    this._block = new Uint8Array(64)
    this._words = new Uint32Array(64)
    this._used = 0
    this._size = 0
    this._digest = null
  }

  get size () {
    return this._size
  }

  update (value) {
    if (this._digest !== null) {
      throw new Error('流式 SHA-256 已完成，不能继续 update')
    }
    const bytes = value instanceof Uint8Array
      ? value
      : value instanceof ArrayBuffer
        ? new Uint8Array(value)
        : null
    if (!bytes) throw new Error('流式 SHA-256 update 仅接受 bytes')
    if (!Number.isSafeInteger(this._size + bytes.byteLength)) {
      throw new Error('流式 SHA-256 输入大小超过安全上限')
    }
    let offset = 0
    while (offset < bytes.byteLength) {
      const length = Math.min(64 - this._used, bytes.byteLength - offset)
      this._block.set(bytes.subarray(offset, offset + length), this._used)
      this._used += length
      this._size += length
      offset += length
      if (this._used === 64) {
        this._compress(this._block)
        this._used = 0
      }
    }
    return this
  }

  digestHex () {
    if (this._digest !== null) return this._digest
    const block = this._block
    block.fill(0, this._used)
    block[this._used] = 0x80
    if (this._used >= 56) {
      this._compress(block)
      block.fill(0)
    }
    let bitLength = BigInt(this._size) * 8n
    for (let index = 63; index >= 56; index -= 1) {
      block[index] = Number(bitLength & 0xffn)
      bitLength >>= 8n
    }
    this._compress(block)
    this._digest = [...this._state]
      .map(word => word.toString(16).padStart(8, '0'))
      .join('')
    return this._digest
  }

  _compress (block) {
    const words = this._words
    for (let index = 0; index < 16; index += 1) {
      const offset = index * 4
      words[index] = (
        (block[offset] << 24) |
        (block[offset + 1] << 16) |
        (block[offset + 2] << 8) |
        block[offset + 3]
      ) >>> 0
    }
    for (let index = 16; index < 64; index += 1) {
      const before15 = words[index - 15]
      const before2 = words[index - 2]
      const sigma0 = rotateRight(before15, 7) ^
        rotateRight(before15, 18) ^ (before15 >>> 3)
      const sigma1 = rotateRight(before2, 17) ^
        rotateRight(before2, 19) ^ (before2 >>> 10)
      words[index] = (
        words[index - 16] + sigma0 + words[index - 7] + sigma1
      ) >>> 0
    }

    let [a, b, c, d, e, f, g, h] = this._state
    for (let index = 0; index < 64; index += 1) {
      const sum1 = rotateRight(e, 6) ^ rotateRight(e, 11) ^ rotateRight(e, 25)
      const choice = (e & f) ^ (~e & g)
      const temp1 = (h + sum1 + choice + roundConstants[index] + words[index]) >>> 0
      const sum0 = rotateRight(a, 2) ^ rotateRight(a, 13) ^ rotateRight(a, 22)
      const majority = (a & b) ^ (a & c) ^ (b & c)
      const temp2 = (sum0 + majority) >>> 0
      h = g
      g = f
      f = e
      e = (d + temp1) >>> 0
      d = c
      c = b
      b = a
      a = (temp1 + temp2) >>> 0
    }
    this._state[0] = (this._state[0] + a) >>> 0
    this._state[1] = (this._state[1] + b) >>> 0
    this._state[2] = (this._state[2] + c) >>> 0
    this._state[3] = (this._state[3] + d) >>> 0
    this._state[4] = (this._state[4] + e) >>> 0
    this._state[5] = (this._state[5] + f) >>> 0
    this._state[6] = (this._state[6] + g) >>> 0
    this._state[7] = (this._state[7] + h) >>> 0
  }
}

export function createStreamingSha256 () {
  return new StreamingSha256()
}
