...
function getBashInlineIntegration (sessionNonce) {
  ...
  return [
    ...
    '__e_old_prompt_command="${PROMPT_COMMAND:-}"',
    '__e_pre() { [[ "${__e_prompting:-0}" == "1" ]] && return; [[ "$BASH_COMMAND" == "__e_"* ]] && return; [[ "${__e_in:-0}" == "0" ]] && { __e_in=1; printf \'\\e]633;E;%s;%s\\a\\e]633;C;%s\\a\' "$__e_nonce" "$(__e_esc "$BASH_COMMAND")" "$__e_nonce"; }; }',
