...
test('real VPS responsiveness gate is credential-safe, read-only and fixed to three rounds', () => {
  ...
  assert.match(source, /remote-directory-cache\.js/)
  assert.match(source, /buildProductionRemoteDirectoryCacheKey/)
  assert.match(source, /sshTerminalPid/)
  assert.match(source, /effectiveUid/)
  assert.match(source, /remoteDirectoryCache\.get\(key\)/)
  assert.match(source, /cacheIsolation\.previousPresent\)\.toBe\(false\)/)
  assert.match(source, /cacheIsolation\.currentPresent\)\.toBe\(true\)/)
  assert.match(source, /acquireCount:\s*3/)
  assert.match(source, /listCount:\s*3/)
  assert.match(source, /releaseCount:\s*3/)
  assert.match(source, /maxActiveCapabilityCount:\s*1/)
  assert.doesNotMatch(source, /remoteDirectoryCache\.stats\(/)
  assert.doesNotMatch(source, /\bcoalesced\b/)
