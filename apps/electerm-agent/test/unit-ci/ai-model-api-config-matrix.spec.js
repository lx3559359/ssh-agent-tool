...
test('P1 model API config matrix covers providers model loading and flexible endpoint paths', () => {
  ...
  assertEvidence(aiConfig, /placeholder='可选，留空自动识别'|placeholder='鍙€夛紝鐣欑┖鑷姩璇嗗埆'/, 'optional API path input')
  ...
  assertEvidence(aiConfigRequiredTest, /does not require optional endpoint path or credentials before sending/, 'optional path and key do not block chat')
