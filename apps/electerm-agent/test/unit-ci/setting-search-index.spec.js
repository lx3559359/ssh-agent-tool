...
test('settings header keeps language preview temporary and preserves all locale options', () => {
  ...
  assert.match(headerSource, /import \{ Button \} from 'antd'/)
  assert.match(headerSource, /className='setting-header-language-select'/)
  assert.match(headerSource, /<option key=\{language\.id\} value=\{language\.id\}>/)
...
test('settings modal rerenders previews and navigates every visible search result through existing store APIs', () => {
  ...
  assert.match(modalSource, /handleChangeSettingTab\(result\.tab\)/)
  assert.match(modalSource, /store\.handleChangeSettingTab\(settingTab\)/)
  ...
  assert.match(modalSource, /shouldHandleSettingsSearchShortcut\(event\)/)
  assert.match(modalSource, /store\.openSetting\(\)/)
  assert.match(headerSource, /role='listbox'/)
  assert.match(headerSource, /role='option'/)
  assert.match(headerSource, /role='combobox'/)
  assert.match(headerSource, /aria-controls=\{searchListboxId\}/)
  assert.match(headerSource, /aria-expanded=\{resultsOpen\}/)
  assert.match(headerSource, /aria-activedescendant=\{activeResultId\}/)
  assert.match(headerSource, /aria-selected=\{index === activeResultIndex\}/)
  assert.match(headerSource, /event\.key === 'ArrowDown'/)
  assert.match(headerSource, /event\.key === 'ArrowUp'/)
  assert.match(headerSource, /event\.key === 'Escape'/)
  assert.match(headerSource, /onSearch\(searchResults\[activeResultIndex\]\)/)
  assert.match(headerSource, /onClick=\{\(\) => onSelectSearchResult\(result\)\}/)
  assert.match(headerSource, /e\(result\.labelKey\)/)
  assert.match(modalSource, /const handleClose = useCallback\(\(\) => \{[\s\S]*?store\.previewLanguage = ''[\s\S]*?store\.hideSettingModal\(true\)[\s\S]*?\}, \[store\]\)/)
  assert.match(modalSource, /<SettingHeader[\s\S]*?onClose=\{onClose\}/)
  assert.match(modalSource, /<SettingModalContent[\s\S]*?onClose=\{handleClose\}/)
