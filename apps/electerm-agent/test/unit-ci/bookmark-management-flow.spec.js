...
test('tree list creates bookmark groups and sub groups through store operations', () => {
  ...
  assert.match(submitBody, /prepareBookmarkGroupCreation\(\{[\s\S]*id: uid\(\)[\s\S]*title: this\.state\.bookmarkGroupTitle/)
  assert.match(submitBody, /window\.store\.addBookmarkGroup\(submission\.group\)/)
  assert.match(submitSubBody, /prepareBookmarkGroupCreation\(submission\)/)
  assert.match(submitSubBody, /prepareBookmarkGroupCreation\(\{[\s\S]*\.\.\.submission,[\s\S]*bookmarkGroups/)
  assert.match(submitSubBody, /bookmarkGroups\.unshift\(prepared\.group\)/)
  assert.match(submitSubBody, /prepared\.parent\.group\.bookmarkGroupIds = prepared\.parent\.bookmarkGroupIds/)
...
test('tree list saves edited bookmark groups', () => {
  ...
  assert.match(body, /prepareBookmarkGroupEdit\(\{[\s\S]*bookmarkGroups,[\s\S]*title: categoryTitle/)
  assert.match(body, /edit\.status === 'invalid'/)
  assert.match(body, /edit\.status === 'missing'/)
  assert.match(body, /edit\.group\.title = edit\.title/)
  assert.match(body, /edit\.group\.color = edit\.color/)
