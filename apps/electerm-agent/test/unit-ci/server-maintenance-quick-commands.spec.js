...
test('high-risk maintenance commands include rollback guidance and safe defaults', async () => {
  ...
  assert.match(firewallText, /FIREWALL_KIND/)
  assert.match(firewallText, /firewalld\|ufw\|iptables\|nftables/)
  assert.match(firewallText, /firewall-rollback\.sh/)
  assert.match(firewallText, /ROLLBACK_SCRIPT\.verified/)
})
...
test('every mutating maintenance command provides a reusable rollback action', async () => {
  ...
  for (const id of mutatingIds) {
    ...
    assert.match(text, /ROLLBACK_SCRIPT|shellpilot-rollback/, `${id} should create a remote rollback script`)
...
test('quick command UI exposes descriptions and edit-before-run confirmation', () => {
  ...
  assert.match(boxSource, /shellpilotQuickFillParameters/)
  assert.match(boxSource, /shellpilotQuickAdvancedEditCommand/)
  assert.match(boxSource, /shellpilotQuickDetectingNetwork/)
  assert.match(boxSource, /shellpilotQuickInterfacesDetected/)
  assert.match(boxSource, /network-interface/)
  assert.match(boxSource, /networkInterfaces/)
  assert.match(boxSource, /shellpilotQuickDetectAgain/)
  assert.match(boxSource, /shellpilotQuickRollbackNow/)
  assert.match(boxSource, /shellpilotQuickRollbackLastChange/)
  assert.match(boxSource, /shellpilotQuickNoRollbackAvailable/)
  assert.match(boxSource, /shellpilotQuickKeepNewConfiguration/)
  ...
  assert.doesNotMatch(boxSource, /createQuickCommandSafetyRecord/)
  assert.match(boxSource, /shellpilotQuickRollback/)
  ...
  assert.match(styleSource, /\.qm-list-wrap[\s\S]*overflow-x hidden/)
  assert.match(styleSource, /@media \(max-width: 1450px\)[\s\S]*grid-template-columns repeat\(3, minmax\(0, 1fr\)\)/)
  assert.match(styleSource, /@media \(max-width: 1100px\)[\s\S]*grid-template-columns repeat\(2, minmax\(0, 1fr\)\)/)
  ...
  assert.match(styleSource, /\.qm-item[\s\S]{0,520}background-image var\(--sp-card-background\)[\s\S]{0,180}border-radius var\(--sp-radius-card\)[\s\S]{0,180}var\(--sp-shadow-card\)/)
  assert.doesNotMatch(styleSource, /\.qm-item[\s\S]{0,320}background var\(--sp-flat-background\)/)
