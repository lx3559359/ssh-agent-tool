...
test('connection server history and AI surfaces use semantic Aurora depth', () => {
  ...
  assert.match(sidebar, /\.sidebar-panel[\s\S]*var\(--sp-shadow-panel\)/)
  assert.match(tree, /\.tree-item[\s\S]*var\(--sp-primary-soft\)/)
  assert.doesNotMatch(tree, /background\s+#000\b/)
  assert.match(ai, /\.chat-history-item[\s\S]{0,420}var\(--sp-shadow-card\)/)
  assert.match(ai, /\.ai-chat-input[\s\S]{0,420}var\(--sp-shadow-control\)/)
})
...
test('Fleet and artifact workspaces reserve strong depth for page containers', () => {
  ...
  assert.match(fleet, /\.fleet-status-toolbar[\s\S]*var\(--sp-shadow-card\)/)
  ...
  assert.match(artifacts, /\.artifact-list-panel[\s\S]*var\(--sp-shadow-panel\)/)
  assert.match(artifacts, /\.artifact-preview[\s\S]*var\(--sp-shadow-panel\)/)
...
test('settings passwords and logs use grouped surfaces instead of per-field cards', () => {
  ...
  assert.match(wrap, /\.setting-header[\s\S]*var\(--sp-panel-background\)[\s\S]*var\(--sp-shadow-lg\)/)
  assert.match(setting, /\.sp-setting-section[\s\S]*var\(--sp-card-background\)[\s\S]*var\(--sp-shadow-card\)/)
  assert.match(setting, /\.setting-passwords[\s\S]*var\(--sp-panel-background\)[\s\S]*var\(--sp-shadow-lg\)/)
  assert.match(info, /\.info-modal[\s\S]*var\(--sp-overlay-background\)[\s\S]*var\(--sp-shadow-overlay\)/)
...
test('data and settings workspaces use large selectable cards and rounded controls', () => {
  ...
  assert.match(font, /\.sp-ui-font-preview[\s\S]{0,420}var\(--sp-shadow-card\)/)
  assertSelectorUsesRadius(theme, '.sp-theme-card', 'card')
  assert.match(theme, /\.sp-theme-card[\s\S]{0,420}var\(--sp-shadow-card\)/)
  assertSelectorUsesRadius(theme, '.sp-theme-preview-scope', 'panel')
  assert.match(setting, /border-radius var\(--sp-radius-control\)/)
  assert.match(setting, /var\(--sp-shadow-control\)/)
})
...
test('Operations Toolkit uses Aurora depth while keeping tool and history rows flat', () => {
  ...
  assert.match(operations, /\.operations-toolkit-workspace[\s\S]*var\(--sp-shadow-panel\)/)
  assert.match(operations, /\.operations-workspace-head[\s\S]*var\(--sp-shadow-control\)/)
  assert.match(operations, /\.operations-recommended-flow[\s\S]*var\(--sp-shadow-card\)/)
