...
test('transfer component integrates skip-aware local upload planning into protected uploads', () => {
  ...
  assert.match(source, /bindRuntimeLocalTransferPlan/)
  assert.match(source, /sharedTransferBatchResultCollector/)
  assert.match(source, /window\.fs\.prepareTransferEntry/)
  assert.match(source, /this\.localSourcePlan\s*=\s*resolveLocalTransferSourcePlan\(props\.transfer\)/)
  assert.match(source, /bindRuntimeLocalTransferPlan\(transfer,\s*nextSourcePlan\)/)
