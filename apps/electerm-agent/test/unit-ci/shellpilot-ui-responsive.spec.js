...
test('ShellPilot top bar and AI panel use theme variables in day and night modes', () => {
  ...
  const topbarShell = topbar.match(/\.aigshell-topbar\r?\n([\s\S]*?)\r?\n\.aigshell-topbar-brand/)
  const panelShell = panel.match(/\.right-side-panel\r?\n([\s\S]*?)\r?\n {2}&\.right-side-panel-pinned/)
...
test('UI font picker stacks and wraps without squeezing labels', () => {
  ...
  assert.match(source, /@media \(max-width: 590px\)[\s\S]*\.sp-ui-font-list[\s\S]*grid-template-columns minmax\(0, 1fr\)/)
