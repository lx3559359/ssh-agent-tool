...
test('windows release workflow publishes only online update assets', () => {
  ...
  assert.match(workflow, /apps\/electerm-agent\/dist\/shellpilot-local\.yml/)
...
test('windows release workflow verifies local update assets before upload', () => {
  ...
  const prepareUpdateAssetsIndex = workflow.indexOf('npm run release:prepare-assets')
  ...
  assert.ok(prepareUpdateAssetsIndex !== -1, 'workflow should prepare online update assets')
  assert.ok(portableBuildIndex !== -1, 'workflow should build the portable package')
  assert.ok(artifactUploadIndex !== -1, 'workflow should upload Windows artifacts')
  assert.ok(prepareUpdateAssetsIndex > portableBuildIndex, 'online update assets should be prepared after all package builds')
  assert.ok(prepareUpdateAssetsIndex < localVerifyIndex, 'online update assets should be prepared before local release verification')
...
test('builds validated local release asset paths only when update files are ready', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      path.join('dist', 'shellpilot-local.yml'),
