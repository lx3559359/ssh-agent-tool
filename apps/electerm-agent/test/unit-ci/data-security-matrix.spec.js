...
test('P0 data security matrix covers encrypted credentials backups and redacted logs', () => {
  ...
  assertEvidence(sqliteSource, /ENC_TABLES = new Set\(\[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*'operationTasks'[\s\S]*\]\)/, 'sqlite safety transaction, artifact, and operation task encryption')
  assertEvidence(nedbSource, /ENC_TABLES = new Set\(\[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*'operationTasks'[\s\S]*\]\)/, 'nedb safety transaction, artifact, and operation task encryption')
  ...
  assertEvidence(safetyTransactionStore, /SQLite and NeDB encrypt safety operations, agent tasks, bounded artifacts, and operation tasks at rest/, 'safety transaction, artifact, and operation task encrypted persistence test')
  assertEvidence(clientDbSource, /export const dbNames = \[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*'operationTasks'[\s\S]*\]/, 'client safety transaction, artifact, and operation task tables')
  assertEvidence(clientDbSource, /export const dbNamesForWatch = \[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*'operationTasks'[\s\S]*\]/, 'local safety transaction, artifact, and operation task watchers')
  ...
  assert.doesNotMatch(syncBlock, /safetyOperations|agentTasks|agentArtifacts|operationTasks/, 'safety audit data must remain local by default')
