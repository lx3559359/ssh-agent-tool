...
test('P0 data security matrix covers encrypted credentials backups and redacted logs', () => {
  ...
  assertEvidence(sqliteSource, /ENC_TABLES = new Set\(\[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*\]\)/, 'sqlite safety transaction and artifact encryption')
  assertEvidence(nedbSource, /ENC_TABLES = new Set\(\[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*\]\)/, 'nedb safety transaction and artifact encryption')
  ...
  assertEvidence(safetyTransactionStore, /SQLite and NeDB encrypt safety operations, agent tasks, and bounded artifacts at rest/, 'safety transaction and artifact encrypted persistence test')
  assertEvidence(clientDbSource, /export const dbNames = \[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*\]/, 'client safety transaction and artifact tables')
  assertEvidence(clientDbSource, /export const dbNamesForWatch = \[[\s\S]*'safetyOperations'[\s\S]*'agentTasks'[\s\S]*'agentArtifacts'[\s\S]*\]/, 'local safety transaction and artifact watchers')
  ...
  assert.doesNotMatch(syncBlock, /safetyOperations|agentTasks|agentArtifacts/, 'safety audit data must remain local by default')
