...
test('artifact workspace provides filters, source preview and responsive layout', () => {
  ...
  assert.match(styles, /\.terminal-workspace-layer\.artifacts-active[\s\S]*visibility hidden/)
  assert.match(styles, /\.artifact-workspace[\s\S]*z-index 90/)
  assert.match(styles, /\.artifact-list-panel[\s\S]*var\(--sp-shadow-panel\)/)
  assert.match(styles, /\.artifact-preview[\s\S]*var\(--sp-shadow-panel\)/)
