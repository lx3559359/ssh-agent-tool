...
test('shared and system menu styles compile with viewport-safe wrapping contracts', async () => {
  ...
  assert.match(shared, /\.ant-dropdown-menu[\s\S]{0,240}max-height calc\(100vh - 24px\)[\s\S]{0,120}overflow-y auto/)
  assert.doesNotMatch(shared, /@media \(max-height: 760px\)[\s\S]*\.ant-dropdown:not\(\.ant-dropdown-menu-submenu-popup\)[\s\S]*inset-block-start 8px !important/)
