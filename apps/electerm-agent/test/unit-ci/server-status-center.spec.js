...
test('server status center uses the approved independent wide-panel layout', () => {
  ...
  assert.match(modal, /shellpilotServerStatusCenter/)
  assert.match(modal, /shellpilotServerStatusPlatformsServices/)
  assert.match(modal, /shellpilotServerStatusFirewallSecurity/)
  assert.match(modal, /shellpilotServerStatusRawResults/)
  assert.match(modal, /shellpilotServerStatusRefreshDetection/)
  assert.match(modal, /shellpilotServerStatusSendToAi/)
  assert.match(modal, /shellpilotServerStatusRecognitionRules/)
  assert.match(modal, /normalizePlatformRules/)
  assert.match(modal, /safeSetItemJSON/)
  assert.match(modal, /buildServerStatusPresentation/)
  assert.match(modal, /server-status-explanation/)
  assert.match(modal, /shellpilotServerStatusImpact/)
  assert.match(modal, /shellpilotServerStatusNextStep/)
  assert.match(modal, /snapshot\.alerts/)
  assert.match(modal, /statusTag\(snapshot\.overallStatus\)/)
  assert.match(modal, /className='server-status-diagnostic-action'/)
  assert.doesNotMatch(modal, /setInterval/)
  assert.match(style, /\.server-status-explanation/)
  assert.match(style, /max-height/)
  assert.match(style, /overflow-y auto/)
  assert.match(style, /@media/)
  assert.doesNotMatch(style, /var\(--sp-text-strong\)/)
})
