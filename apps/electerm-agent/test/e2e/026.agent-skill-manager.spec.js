...
test('isolated Skill manager covers conversational draft, review, selection and rollback', async () => {
  ...
  try {
    ...
    const summary = creator.getByTestId('agent-skill-draft-summary')
    await expect(summary).toBeVisible()
    await expect(summary).toContainText('Inspect Web Service')
    await expect(summary).toContainText('web service health')
    await expect(creator.getByTestId('agent-skill-technical-details')).not.toBeVisible()
    await expect(creator.locator('.agent-skill-editor')).toHaveCount(0)
    await creator.locator('.agent-skill-draft-review .ant-collapse-header').click()
    await expect(creator.getByTestId('agent-skill-technical-details')).toBeVisible()
    await expect(creator.locator('.agent-skill-editor')).toBeVisible()
    await creator.locator('.agent-skill-draft-review .ant-collapse-header').click()
    await expect(creator.locator('.agent-skill-editor')).toHaveCount(0)
    ...
    await creator.getByTestId('agent-skill-save-draft').click()
